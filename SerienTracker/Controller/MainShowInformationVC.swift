...
class MainShowInformationVC: UIViewController {
    ...
    @objc func bookmarkTapped() {
        ...
        showMainAPI.getShowOverview(id: (showInfo?.show?.id)!) { [weak self] show in
            guard let show = show else { fatalError("No show found") }
            ...
            self?.showEpListAPI.getEpList(id: (self?.showInfo?.show?.id)!, complition: { [weak self] episoden in
                ...
                if let showObject = self?.realm.object(ofType: RealmBookmarkShow.self, forPrimaryKey: show.showId) {
                    self?.deleteBookmarkShow(show: show, realmShow: showObject)
                } else {
                    self?.saveAsBookmarkShow(show: show, episodes: episoden!)
                }
...
extension MainShowInformationVC {
    func saveAsBookmarkShow(show: ShowMainInformation, episodes: [ShowEpisodenInformation]) {
        do {
            try realm.write {
    ...
    func deleteBookmarkShow(show: ShowMainInformation, realmShow: RealmBookmarkShow) {
        let showImageObject = realm.object(ofType: RealmShowImage.self, forPrimaryKey: show.showId)
        do {
            try realm.write {
                self.realm.delete(showImageObject!)
                // self?.realm.delete(episodenImageObject!)
                let episodeShowImages = self.realm.objects(RealmEpImage.self).filter("showId==\(show.showId)")
